...
export const updateCityOffers = (hotels, city) => ({
...
export const loadHotels = () => (dispatch, _getState, api) => (
  api.get(APIRoutes.HOTELS)
    .then(({data}) => {
      ...
      const cityOffers = getHotelsOfCity(hotels, _getState().city);
      dispatch({ type: ActionTypes.LOAD_HOTELS, payload: hotels });
      dispatch({ type: ActionTypes.CHANGE_OFFERS_OF_CITY, payload: cityOffers });
    })
);
